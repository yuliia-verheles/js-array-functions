...
describe('getPhones function', () => {
    test('return new array (not mutated) of objects', () => {
        const testNewPhones = getPhones(phones);
        expect(testNewPhones).not.toBe(phones);
        //expect(testNewPhones).not.toStrictEqual(phones); //???????????????????        
    });
});